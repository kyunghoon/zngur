...
pub fn map_type_paths(ty: Type, on_typepath: &mut impl FnMut(TypePath) -> TypePath, lts: Option<&HashSet<Ident>>) -> Type {
    match ty {
        Type::Array(TypeArray { bracket_token, elem, semi_token, len }) => 
            Type::Array(TypeArray { bracket_token, elem: Box::new(map_type_paths(*elem, on_typepath, lts)), semi_token, len }),
        Type::BareFn(TypeBareFn { lifetimes, unsafety, abi, fn_token, paren_token, inputs, variadic, output }) =>
            Type::BareFn(TypeBareFn { lifetimes, unsafety, abi, fn_token, paren_token,
                inputs: inputs.into_iter().map(|mut i| { i.ty = map_type_paths(i.ty, on_typepath, lts); i }).collect(),
                variadic, 
                output: match output {
                    ReturnType::Type(rarrow, ty) => ReturnType::Type(rarrow, Box::new(map_type_paths(*ty, on_typepath, lts))),
                    x => x
            }
        }),
        Type::Group(TypeGroup { group_token, elem }) => 
            Type::Group(TypeGroup { group_token, elem: Box::new(map_type_paths(*elem, on_typepath, lts)) }),
        Type::Paren(TypeParen { paren_token, elem }) =>
            Type::Paren(TypeParen { paren_token, elem: Box::new(map_type_paths(*elem, on_typepath, lts)) }),
        Type::Path(mut type_path) => {
                        ...
                        gargs.args = gargs.args.into_iter().filter_map(|a| {
                            match a {
                                ...
                                GenericArgument::Type(ty) =>
                                    Some(GenericArgument::Type(map_type_paths(ty, on_typepath, lts))),
                                _ => None,
                            }
                        }).collect();
            ...
            let type_path = on_typepath(type_path);
            Type::Path(type_path)
        },
        Type::Ptr(TypePtr { star_token, const_token, mutability, elem }) =>
            Type::Ptr(TypePtr { star_token, const_token, mutability, elem: Box::new(map_type_paths(*elem, on_typepath, lts)) }),
        Type::Reference(TypeReference { and_token, lifetime, mutability, elem }) =>
            Type::Reference(TypeReference {
                ...
                mutability,
                elem: Box::new(map_type_paths(*elem, on_typepath, lts))
            }),
        Type::Slice(TypeSlice { bracket_token, elem }) =>
            Type::Slice(TypeSlice { bracket_token, elem: Box::new(map_type_paths(*elem, on_typepath, lts)) }),
        Type::Tuple(TypeTuple { paren_token, elems }) =>
            Type::Tuple(TypeTuple { paren_token, elems: elems.into_iter().map(|e| map_type_paths(e, on_typepath, lts)).collect() }),
        x => x
    }
}
...
pub fn to_fully_qualified(ty: Type, to_full: Option<&Path>, ty_env: Option<&TypeEnv>, dbg: bool, lts: Option<&HashSet<Ident>>) -> Type {
    map_type_paths(ty, &mut |type_path| {
